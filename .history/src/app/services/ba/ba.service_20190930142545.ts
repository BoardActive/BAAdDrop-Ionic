...
import { PlacesDto } from '../../models/places.model';
import { MessagesDto } from '../../models/messages.model';
import { LocalStorageService } from '../../services/local-storage/local-storage.service';
import { MeDto } from '../../models/me.model';

export const Config = {
  API_URL: {
    APP_URL_PROD: 'https://api.boardactive.com/web/v1',
    APP_URL_DEV: 'https://springer-api.boardactive.com/web/v1',
  }
};
...
export class BAService {
  ...
  public getEnvironment(): Observable<any> {
    ...
    return new Observable((observer) => {
      this.storageService.getItem('environment').subscribe(env => {
  ...
  //   return new Observable(observer => {
  //     this.getEnvironment().subscribe(setUrl => {
  //       const url = setUrl + '/auth-config/google';
  ...
  public getApp(id: any): Observable<any> {
    return new Observable(observer => {
      this.getEnvironment().subscribe(setUrl => {
        const url = setUrl + '/apps/' + id;
        console.log(`+++`);
        this.http
          .get(url, {
            headers: {},
  ...
  public getBilling(): Observable<any> {
    return new Observable(observer => {
      this.getEnvironment().subscribe(setUrl => {
        const url = setUrl + '/billing-plans';
        console.log(`+++`);
        this.http
          .get(url, {
            headers: {},
