...
export class BoardActiveService {
    ...
    */
     ...
     * Cordova Plugin: cordova-plugin-device
     * /********************************************************x*x***************************************
     * Cordova Plugin: cordova-plugin-badge
     * Get, Set and Clear App Icon badge
     */

    /**
     * Get Number of current Badges
     */
    async getBadges() {
        try {
            const badge = await this.badge.get();
            console.log(`badge: getBadges(): ${badge}`);
        } catch (e) {
            console.error(`badge: getBadges(): ${e}`);
        }
    }

    /**
     * Request Permission to manage Badges
     */
    async requestPermission() {
        try {
            const permission = await this.badge.requestPermission();
            console.log(`badge: requestPermission(): ${permission}`);
        } catch (e) {
            console.error(`badge: requestPermission(): ${e}`);
        }
    }
     ...
     * Set Badges
     */
    async setBadges(val?: number) {
        try {
            const badge = await this.badge.set(val = val || 1);
            console.log(`badge: setBadges(): ${badge}`);
        } catch (e) {
            console.error(`badge: setBadges(): ${e}`);
        }
    }
     ...
     * Increase Badges
     */
    async increaseBadges(val?: number) {
        try {
            const badge = await this.badge.increase(val = val || 1);
            console.log(`badge: increaseBadges(): ${badge}`);
        } catch (e) {
            console.error(`badge: increaseBadges(): ${e}`);
        }
    }
    ...
    async decreaseBadges(val?: number) {
        try {
            const badge = await this.badge.decrease(val = val || 1);
            console.log(`badge: decreaseBadges(): ${badge}`);
        } catch (e) {
            console.error(`badge: decreaseBadges(): ${e}`);
        }
    }
    ...
    async clearBadges() {
        try {
            const badge = await this.badge.clear();
            console.log(`badge: clearBadges(): ${badge}`);
        } catch (e) {
            console.error(`badge: clearBadges(): ${e}`);
        }
    }
