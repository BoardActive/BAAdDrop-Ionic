...
public easteregg: number = 0;
  ...
  signIn() {
    ...
    this.baService.postLogin(this.email, this.password).subscribe(response => {
      ...
      if (appModel.apps) {
        this.storage.set('auth', true).then((response) => {
          console.log(`apps: ${JSON.stringify(appModel.apps, null, 2)}`);
          this.localStorageService.setItem('apps', appModel.apps).subscribe(() => {
