...
import { Platform, NavParams, Events, AlertController, IonContent } from '@ionic/angular';
...
export class BaMessagesPage implements OnInit {
  ...
  searchEvents(ev: any) {
    this.log_events = this.filterItems(this.searchTerm);
  }

  filterItems(searchTerm) {
    return this.log_events.filter((item) => {
  ...
  */
  ...
  getData() {
    ...
    this.localStorageService.getItem('messages').subscribe(data => {
      this.messages = data;
      this.messages = this.messages.sort((a,b) => new Date(b.dateCreated) - new Date(a.dateCreated));
