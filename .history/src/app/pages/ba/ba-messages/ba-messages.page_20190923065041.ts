...
*/
  ...
  */
  ...
  getMessages() {
    this.localStorageService.getItem('messages').then(data => {
      this.message = data;
      console.log(`getMessages: ${JSON.stringify(this.message, null, 2)}`);
    });
  }
  ...
  welcomeMsg() {
    ...
    // messages = [];
    // let msgCnt: any = this.localStorageService.getItem('msgCnt');
    // if (msgCnt) {
    //   msgCnt++;
    // } else {
    //   msgCnt = 1;
    ...
    // msg.imageUrl = 'https://ba-us-east-1-develop.s3.amazonaws.com/test-5d3ba9d0-cb1d-49ee-99f3-7bef45994e71';    
    // messages.push(msg);
    // this.localStorageService.setItem('messages', messages);
    // this.localStorageService.setItem('msg', msg);
    // this.baService.newLocalNotification(msg, 1);
  }

  getMsg() {
    const msg = this.localStorageService.getItem('msg');
  ...
  clearMsg() {
    ...
    this.localStorageService.removeItem('message');
  }
